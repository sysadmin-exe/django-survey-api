...
# @api_view(['GET', 'POST'])
# # API Method - Function that creates a survey
# def surveyCreate(request):
#     if request.method == 'POST':
#         survey_data = JSONParser().parse(request)
#         surveyserializer = SurveyQuestionSerializer(data=survey_data)
...
# class SurveyOption(generics.ListCreateAPIView):
#     queryset = SurveyOption.objects.all()
#     serializer_class = SurveyOptionSerializer
    
# @api_view(['GET', 'POST'])
# # API Method - Function that creates a survey
# def SurveyOptions(request):
#     if request.method == 'POST':
#         survey_data = JSONParser().parse(request)
#         surveyserializer = SurveyOptionSerializer(data=survey_data)
#         if surveyserializer.is_valid():
#             surveyserializer.save()            
#             return JsonResponse(surveyserializer.data, status=status.HTTP_201_CREATED) 
#         return JsonResponse(surveyserializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
# class SurveyAnswer(generics.ListCreateAPIView):
